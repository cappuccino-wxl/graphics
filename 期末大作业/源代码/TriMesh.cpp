...
glm::mat4 TriMesh::getModelMatrix2()
{

	return ModelModelMatrix;
}

void TriMesh::settModelMatrix(glm::mat4 m)
{
	ModelModelMatrix = m;
}
...
void TriMesh::generateCube2(glm::vec3 _color)
{
	// 创建顶点前要先把那些vector清空
	cleanData();
...
void TriMesh::readObj(const std::string &filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		if (type == "vn")
		{
			...
			a = float(std::min(std::max(0.5 * (_x + 1.0), 0.0), 1.0));
			b = float(std::min(std::max(0.5 * (_y + 1.0), 0.0), 1.0));
			c = float(std::min(std::max(0.5 * (_z + 1.0), 0.0), 1.0));
			// 用法向导出颜色分量
			vertex_colors.push_back(glm::vec3(a, b, c));
		}
		if (type == "f")
		{
			float a, b, c;
...
void TriMesh::readObj2(const std::string &filename)
{
	std::ifstream fin(filename);
	std::string line;
	...
	while (std::getline(fin, line))
	{
		...
		char slash;
		sin >> type; // 根据obj文件中不同的类型读取数据
		/*vertex_textures中存储UV坐标数据，vertex_positions存储顶点坐标数据
		vertex_normals存储顶点法线数据
		vertex_colors存储顶点颜色数据；
		faces存储三角面片的顶点索引数据，normal_index存储三角面片的顶点的法向量数据的索引下标
		texture_index存储三角面片的顶点的纹理坐标数据的索引下标。
		因obj文件无法存储颜色数据，可以使用法线作为颜色数值；
		参考实验4.1中程序生成物体的内容，确定不同数据的存储位置和方法。*/
		if (type == "v")
		{
			sin >> _x >> _y >> _z;
			vertex_positions.push_back(glm::vec3(_x, _y, _z));
		...
		if (type == "f")
		{
			...
			faces.push_back(vec3i(a2 - 1, a3 - 1, a0 - 1));
			// 纹理坐标
			texture_index.push_back(vec3i(b2 - 1, b3 - 1, b0 - 1));
			// 法向下标索引
			normal_index.push_back(vec3i(c2 - 1, c3 - 1, c0 - 1));
			// 颜色下标索引
			color_index.push_back(vec3i(c2 - 1, c3 - 1, c0 - 1));
		}
